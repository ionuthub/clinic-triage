...
export function ReferralListPage() {
  ...
  const filters = useReferralFilters(referrals);
  ...
  const booked = referrals.filter((item) => item.status === "booked").length;
  return (
    <div className="page">
      <header className="page-header">
        <div>
          ...
          <p>
            Review clinical information, assign a priority and coordinate the
            next appointment.
          </p>
        </div>
      ...
      <section className="metrics" aria-label="Referral summary">
        <div className="metric">
          ...
          <small>Requires triage decision</small>
        </div>
        <div className="metric accent">
          <span>Time sensitive</span>
          <strong>{urgent}</strong>
          <small>Urgent or safeguarded</small>
        </div>
        <div className="metric">
        ...
        <div className="metric">
          ...
          <small>Current working list</small>
        </div>
      </section>
        ...
        <div className="filters">
          <label className="search-field">
            ...
            <input
              ...
              placeholder="Search patient, NHS number or service"
            />
          </label>
          <select
          ...
          <select
            ...
            value={filters.band}
            onChange={(event) =>
              filters.setBand(event.target.value as typeof filters.band)
            }
          >
            <option value="all">All priorities</option>
            <option value="red">Priority 1</option>
            <option value="amber">Priority 2</option>
            <option value="green">Priority 3</option>
            <option value="unbanded">Unbanded</option>
          </select>
          {filters.activeCount > 0 && (
            <button className="text-button" onClick={filters.reset}>
              Clear {filters.activeCount}
            </button>
          )}
        </div>
        <div className="list-summary">
          <strong>{filters.filtered.length} referrals</strong>
          <span>Sorted by most recently received</span>
        </div>
