...
interface ClinicState {
  ...
  audit: AuditEntry[];
  actor: string;
  acceptReferral: (id: string, override?: PriorityBand) => void;
  ...
  setPriority: (id: string, band: PriorityBand) => void;
  addAudit: (entry: Omit<AuditEntry, "id" | "occurredAt">) => void;
}
const clone = <T>(value: T): T => structuredClone(value);
export const useClinicStore = create<ClinicState>((set, get) => ({
  referrals: clone(seedReferrals),
  clinicians: clone(seedClinicians),
  appointments: [],
  ...
  acceptReferral: (id, override) => {
    const referral = get().referrals.find((item) => item.id === id);
    ...
    set((state) => ({
      referrals: state.referrals.map((item) =>
        item.id === id ? { ...item, status: "accepted", priority: band } : item,
      ),
    }));
    get().addAudit({
      actor: get().actor,
      ...
      severity: band === "red" ? "critical" : "info",
    });
    clinicEvents.emit("referral:accepted", {
      referralId: id,
      band,
      actor: get().actor,
    });
  ...
  setPriority: (id, band) => {
    set((state) => ({
      referrals: state.referrals.map((item) =>
        item.id === id ? { ...item, priority: band } : item,
      ),
    }));
    get().addAudit({
      actor: get().actor,
      ...
      referralId: id,
      detail: `Priority changed to ${band}`,
      severity: "info",
    });
  },
  bookAppointment: (referralId, clinicianId, slotId) => {
    ...
    get().addAudit({
      ...
      referralId,
      detail: `${clinician.name} · ${new Date(appointment.startsAt).toLocaleString()}`,
      severity: "info",
    });
    return appointment;
  },
